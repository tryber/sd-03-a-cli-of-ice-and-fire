...
const getBooksFromPage = async (pageLink, name) =>
  new Promise((resolve, reject) => {
    superagent.get(`${pageLink}?name=${name}`).end((err, response) => {
      console.log(`${pageLink}?name=${name}`);
...
const removePropertiesFromBook = ({ books, povBooks, ...book }) => removeEmptyProperties(book);
 ...
 * @param {Function} goBackBooksMenu Função que exibe o menu de personagens
 * @param {string} pageLink Link da página a ser exibida.
 */
const searchBook = async (pageLink) => {
  try {
    ...
    const apiResponse = await getBooksFromPage(pageLink || FIRST_PAGE_LINK, searchName.name);
    // console.log(apiResponse)
    if (searchName.name === '' || apiResponse.books.length < 1) {
      const fullList = await getBooksFromPage(pageLink || FIRST_PAGE_LINK, '');
      return fullList;
    }
    return apiResponse;
  } catch (err) {
...
const showBooksList = async (goBackBooksMenu, pageLink) => {
  const { books, links } = await searchBook(pageLink);
  ...
  if (nextAction === 'repeat') {
    return showBooksList(goBackBooksMenu);
  }
