 ...
 */
async function showBooksList(goBackToCharactersMenu, pageLink) {
  const bookName = await getBookName('Digite o nome de um livro :');
  console.log(bookName);
  if (!bookName) {
    const { books, links } = await getBooksFromPage(ALL_BOOKS_ENDPOINT);
  } else {
    const { books, links } = await getBooksFromPage(bookNameUrl(bookName));
  }
  // console.log(teste);
  // const { books, links } = await getBooksFromPage(pageLink || ALL_BOOKS_ENDPOINT);
