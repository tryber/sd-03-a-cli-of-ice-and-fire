...
const createChoiceFromCharacter = (characters) => ({
  ...
  name: book[0].name,
  value: removePropertiesFromCharacter(characters),
});
...
/* const removePropertiesFromBooks = ({ characters, povCharacters, name }) */

const bookApiWithName = 'https://www.anapioficeandfire.com/api/books?name=';

const getBookFromSearch = (url, bookName) => {
  return superagent.get(url + bookName)
};
...
const showBookList = async (goBackToBooksMenu, pageLink) => {
