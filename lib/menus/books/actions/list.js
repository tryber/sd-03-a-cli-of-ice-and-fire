...
const showBooksList = async (goBackToBooksMenu, listLink) => {
  try {
    ...
    await showBooksList(goBackToBooksMenu, PAGE_LINK());
  } catch (error) {
    /* Se digitar pontuação ou outro símbolo na pesquisa a promise pode ser rejeitada
    alguma função em utils deve quebrar por isso, vou investigar qual */
    console.log('Erro na aplicação, parametro inválido');
    return goBackToBooksMenu();
  }
};
