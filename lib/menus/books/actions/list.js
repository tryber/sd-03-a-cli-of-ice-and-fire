...
const removePropertiesFromBooks = ({ books, povBooks, ...character }) =>
...
const createChoicesList = (characters, links) => {
  const choices = characters.map(createChoiceFromBooks);
 ...
 */
const handleUserC = ( userChoice, {goBackToBooksMenu: goBackToBookMenu, showBooksList, links }) => {
  if (userChoice === 'back') return goBackToBookMenu();
  if (userChoice === 'next' || userChoice === 'prev') {
     ...
     */
    return showBooksList(goBackToBookMenu, links[userChoice]);
  }
...
const showBooksList = async (goBackToBooksMenu, pageLink) => {
  ...
  await handleUserC(userChoice, {
    goBackToCharactersMenu: goBackToBooksMenu,
    showCharactersList: showBooksList,
    links,
  });
