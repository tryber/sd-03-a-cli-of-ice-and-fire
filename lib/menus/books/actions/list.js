...
//   const books = await superagent.get(pageLink);
//   const characters = books.body;
//   const links = parseLinks(books.headers.link);
...
//   const book = await superagent.get(pageLink);
//   return book.body;
// };
...
const searchBookForName = async () => {
  ...
  if (title !== '') {
    await getBook(`${FIRST_PAGE_LINK}${title}`)
      .then((bookInfo) =>
        (bookInfo[0]
...
const showCharactersList = async (goBackToCharactersMenu, pageLink) => {
  if (!pageLink) await searchBookForName();

  const books = await getBooksFromPage(pageLink || FIRST_PAGE_LINK);
  const { characters, links } = books;
