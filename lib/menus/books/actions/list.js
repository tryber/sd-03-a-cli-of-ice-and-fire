...
const createChoicesList = (books, links) => {
  const choices = books.map((book) => createChoiceFromCharacter(book));
...
const showBooksList = async (goToBackBooksMenu, linksList) => {
  ...
  const { books, links } = await getBooks(linksList || MENU_BOOKS_LIST(inputUser));
  console.log('show link: ', links);
