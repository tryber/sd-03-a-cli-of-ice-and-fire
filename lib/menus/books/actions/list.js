 ...
 */
const showBookList = async (goBackToCharactersMenu, pageLink) => {
  const userInput = inquirer.prompt([
    { name: 'bookName', type: 'input', message: 'Digite o nome de um livro: '}
  ]).then((answers) => {
    superagent.get(`https://www.anapioficeandfire.com/api/books?name=${answers}`)
    .then((resApi) => console.log(resApi))
  }).catch(console.error)
