...
let PAGE_LINK = 'https://www.anapioficeandfire.com/api/books?page=1&pageSize=10&name=';
...
const showBooksList = async (goBackToCharactersMenu, pageLink) => {
  ...
  if (!pageLink) BOOK_LINK = await searchBook()
    .then((book) => `${SEARCH_PAGE_LINK}${book}`);
  
  const { characters, links } = await getBooksFromPage(pageLink || BOOK_LINK || PAGE_LINK);
