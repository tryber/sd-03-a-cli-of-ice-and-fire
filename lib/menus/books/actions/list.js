...
const handleUserChoice = (userChoice, { goBackToBooksMenu, showBooksSearch, links, pageLink }) => {
  ...
  return showBooksSearch(goBackToBooksMenu, pageLink);
};

const showBooksSearch = async (goBackToBooksMenu, pageLink) => {
  const answer = pageLink || await getUsersChoice();

  const { books, links } = await getBookFromPage(pageLink || `${BOOKS_SEARCH_LINK}${answer}`);
