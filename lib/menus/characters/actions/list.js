...
/* As written at address https://www.anapioficeandfire.com/Documentation#books. Personal clarification Roger.
*/
...
const getCharactersFromPage = (pageLink) =>
  new Promise((resolve, reject) => {
    superagent.get(pageLink).end((err, response) => {
      ...
      const links = parseLinks(response.headers.link); /* lower menu,
go back to the previous page or the nexto one. Personal clarification Roger.
*/
...
const createChoiceFromCharacter = (character) => ({
  ...
  name: character.name || character.aliases[0], // Show the character name
  value: removePropertiesFromCharacter(character),
});

const createChoicesList = (characters, links) => { // take the link and transform to character
