...
const getHousesFromPage = (pageLink) =>
  new Promise((resolve, reject) => {
    superagent.get(pageLink, (err, response) => {
 ...
 * @param {Function} goBackToHouseMenu Função que exibe o menu de personagens
 * @param {string} pageLink Link da página a ser exibida.
 */
const showHousesList = async (goBackToHouseMenu, pageLink) => {
  ...
  if (nextAction === 'back') {
    return goBackToHouseMenu();
  }

  if (nextAction === 'repeat') {
    return showHousesList(goBackToHouseMenu);
  }
