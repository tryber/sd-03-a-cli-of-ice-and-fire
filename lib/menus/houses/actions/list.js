...
const getHousesFromPage = async (pageLink) => {
  try {
    const returnAPI = await superagent.get(pageLink);
    const houses = returnAPI.body;
    const links = parseLinks(returnAPI.headers.link);
...
const removePropertiesFromHouses = ({ houses, povHouses, ...house }) =>
