...
const MENU_MAIN_MESSAGE = '[Listar Casas] - Escolha uma personagem para ver mais detalhes';
...
const removePropertiesFromHouse = ({ words, titles, seats, currentLord, founded, founder, diedOut, ancestralWeapons, cadetBranches, swornMembers, ...house }) =>
...
const afterHouseDisplay = async (goBackToHousesMenu) => {
  const action = await showMenuOptions({
    message: 'O que deseja fazer agora?',
    choices: [
      { name: 'Voltar para o menu principal', value: 'back' },
      { name: 'Exibir outra casa', value: 'list' },
      { name: 'Sair', value: 'exit'},
    ],
  });

  if (action === 'back') return goBackToHousesMenu();
  if (action === 'list') return showHousesList(goBackToHousesMenu);
...
const handleUserChoice = (userChoice, { goBackToHousesMenu, showHousesList, links }) => {
  ...
  afterHouseDisplay(goBackToHousesMenu);
};
