...
const handleUserChoice = (userChoice, { goBackToHousesMenu, showHousesData, links }) => {
  if (userChoice === 'back') return goBackToHousesMenu();
  if (userChoice === 'next' || userChoice === 'prev') {
    return showHousesData(goBackToHousesMenu, links[userChoice]);
  }
...
const showHousesData = async (goBackToHousesMenu, pageLink) => {
  ...
  if (nextAction === 'repeat') {
    return showHousesData(goBackToHousesMenu);
  }
...
module.exports = { run: showHousesData };
